...
- `NotesStructured` — **the single source of truth for notes content**: `title: String?`, `summary: String`, `detailedNotes: String` (long-form markdown body), `decisions: [String]`, `actionItems: [ActionItem]`, `userActionItems: [ActionItem]` (renamed from `legacyUserActionItems`; wire/schema key `user_action_items`, G4), with `ActionItem = {owner: String, text: String}`.
