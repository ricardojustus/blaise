    ...
    func snapshots(from start: Date, to end: Date) async -> [CalendarEventSnapshot] {
        ...
        do {
            let events = try await client.snapshots(
                clientID: normalized, refreshToken: refreshToken, from: start, to: end)
            connected = true
            lastError = nil
            lastRefresh = Date()
            return events
...
actor GoogleCalendarClient {
    ...
    func authorize(clientID: String) async throws -> String {
        ...
        let codeVerifier = Self.randomURLSafe(byteCount: 48)
    ...
    func snapshots(
        clientID: String, refreshToken: String, from start: Date, to end: Date
        ...
        let calendarIDs = calendars.isEmpty ? ["primary"] : calendars.map(\.id)
