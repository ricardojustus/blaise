...
public enum NotesRenderer {
    ...
    ///
    /// `annotations`: the meeting's correction rows. Only `.annotation` rows
    /// weave (callers pass the whole set; understanding corrections act
    ...
    public static func render(
        ...
        blocks.append(
            contentsOf: weave.userActionAsides.map { aside($0.text, on: $0.quote, strings: strings) })
    ...
    struct AnnotationWeave {
        ...
        static func plan(annotations: [MeetingCorrection], structured: NotesStructured) -> AnnotationWeave {
            ...
            for row in annotations where row.kind == .annotation {
                ...
                case .userActionItem:
                    weave.userActionAsides.append((blocks[hit.blockIndex], row.userText))
                }
            }
            return weave
        }
    ...
    struct LocalizedStrings {
        ...
        /// The anchored-aside tag, the unanchored tail heading, and the
    ...
    /// of a title containing U+2028. User correction text has its own fold
    /// (`CorrectionSanitize.flatten`), which is where that hardening belongs.
